...
router.get("/setup-db", async (req, res) => {
  try {
    ...
    const { error: templateTableError } = await supabase.from("templates").select("id").limit(1)
    ...
    res.json({
      ...
      setupSteps: ["1. Go to Supabase SQL editor", "2. Execute the following SQL:", ...addColumnsSQL],
    })
  } catch (error) {
...
router.get("/public", (req, res, next) => {
  try {
    return testController.publicTest(req, res, next)
  } catch (err) {
    next(err)
  }
})
...
router.get("/protected", auth, (req, res, next) => {
  try {
    return testController.protectedTest(req, res, next)
  } catch (err) {
    next(err)
  }
})
...
router.get("/admin", auth, (req, res, next) => {
  try {
    return testController.adminTest(req, res, next)
  } catch (err) {
    next(err)
  }
})
