...
router.get("/", async (req, res) => {
  try {
    let query = supabase.from("templates").select("*, users(display_name, avatar_url)")
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    if (questions.length > 0) {
      ...
      const { error: questionsError } = await supabase.from("questions").insert(questionsWithTemplateId)
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (questions) {
      ...
      const { error: deleteError } = await supabase.from("questions").delete().eq("template_id", id)
      ...
      if (questions.length > 0) {
        ...
        const { error: insertError } = await supabase.from("questions").insert(questionsWithTemplateId)
...
router.post("/:id/duplicate", authenticateToken, async (req, res) => {
  try {
    ...
    if (questions && questions.length > 0) {
      ...
      const { error: insertQuestionsError } = await supabase.from("questions").insert(newQuestions)
...
router.post("/:id/like", authenticateToken, async (req, res) => {
  try {
    ...
    if (existingLike) {
      ...
      const { error: unlikeError } = await supabase.from("template_likes").delete().eq("id", existingLike.id)

      if (unlikeError) throw unlikeError
    } else {
      // User hasn't liked this template yet, so like it
      const { error: likeError } = await supabase.from("template_likes").insert({
        template_id: id,
        user_id: req.user.id,
      })
