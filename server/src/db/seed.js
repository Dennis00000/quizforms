...
async function seedCategories() {
  ...
  for (const category of categories) {
    ...
    if (!data || data.length === 0) {
      const { error: insertError } = await supabaseAdmin.from("categories").insert(category)
...
async function seedUsers(count) {
  ...
  const { data: existingUsers, error: checkError } = await supabaseAdmin.from("profiles").select("id").limit(count)
...
async function seedTemplates(userIds, count) {
  ...
  // Fetch categories
  const { data: categories, error: catError } = await supabaseAdmin.from("categories").select("id")
  ...
  for (let i = 0; i < count; i++) {
    ...
    const { data: newTemplate, error: templateError } = await supabaseAdmin.from("templates").insert(template).select()
    ...
    for (const category of selectedCategories) {
      await supabaseAdmin.from("template_categories").insert({
...
async function seedQuestionsForTemplate(templateId) {
  ...
  for (let i = 0; i < questionCount; i++) {
    ...
    if (type === "choice" || type === "multiple_choice") {
      ...
      options = Array.from({ length: optionCount }, (_, i) => faker.lorem.words(Math.floor(Math.random() * 3) + 1))
    } else if (type === "scale") {
      options = {
        min: 1,
        max: 5,
    ...
    await supabaseAdmin.from("questions").insert(question)
  }
}
...
async function seedSubmissions(templateIds, userIds, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const { error: submissionError } = await supabaseAdmin.from("submissions").insert(submission)
...
function generateAnswerForQuestion(question) {
  switch (question.type) {
    ...
    case "choice":
      ...
      return choiceOptions.length > 0 ? choiceOptions[Math.floor(Math.random() * choiceOptions.length)] : null
    case "multiple_choice":
      const mcOptions = JSON.parse(question.options || "[]")
      const selectedCount = Math.floor(Math.random() * mcOptions.length) + 1
      return [...mcOptions].sort(() => 0.5 - Math.random()).slice(0, selectedCount)
    case "date":
