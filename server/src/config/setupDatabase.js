...
const setupDatabase = async () => {
  try {
    ...
    const { error: tableError } = await serviceSupabase.from("users").select("id").limit(1)
    ...
    const { data: columns, error: columnsError } = await serviceSupabase.from("users").select().limit(1)
...
const ensureTemplatesColumns = async () => {
  ...
  // Get current columns
  const { data: columns, error } = await serviceSupabase.from("templates").select().limit(1)
