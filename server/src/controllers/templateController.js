  ...
  toggleLike = async (req, res, next) => {
    try {
      ...
      // Otherwise, add a like
        ...
        const { error: addLikeError } = await supabase.from("template_likes").insert({
          template_id: id,
          user_id,
        })
      ...
      const { error: updateError } = await supabase.from("templates").update({ likes_count: likeCount }).eq("id", id)
