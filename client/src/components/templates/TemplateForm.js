...
const TemplateForm = ({ initialData = null }) => {
  ...
  const onDrop = useCallback(
    async (acceptedFiles) => {
      ...
      try {
        ...
        const { data, error } = await supabase.storage.from("template-images").upload(fileName, file)

        if (error) throw error

        const { publicURL } = supabase.storage.from("template-images").getPublicUrl(data.path)
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold mb-6">{initialData ? t("templates.edit") : t("templates.create")}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Input label={t("templates.title")} name="title" value={formData.title} onChange={handleChange} required />
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-500">{t("templates.publicDescription")}</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t("templates.image")}</label>
          <div
            {...getRootProps()}
          ...
          >
            <div className="space-y-1 text-center">
              <input {...getInputProps()} />
              {formData.imageUrl ? (
                <img src={formData.imageUrl} alt="Template" className="mx-auto h-32 w-auto" />
              ) : (
        ...
        <TagInput label={t("templates.tags")} tags={formData.tags} onChange={handleTagsChange} />

        <QuestionBuilder questions={formData.questions} onChange={handleQuestionsChange} />
