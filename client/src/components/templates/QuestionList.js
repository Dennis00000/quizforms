...
const QuestionList = ({ questions, templateId, onQuestionsChange, isEditable = false }) => {
  ...
  const handleDelete = async (questionId) => {
    try {
      const { error } = await supabase.from("questions").delete().eq("id", questionId).eq("template_id", templateId)
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white">{t("templates.questions")}</h3>
        <button
          type="button"
          onClick={addQuestion}
