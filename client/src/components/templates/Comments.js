      ...
      <div ref={commentListRef} className="space-y-4">
        {loading ? (
          ...
          comments.map((comment) => (
            <div key={comment.id} className="bg-white dark:bg-gray-800 shadow rounded-lg p-4">
              <div className="flex space-x-3">
                <div className="flex-shrink-0">
                  <img className="h-10 w-10 rounded-full" src={comment.user.avatar || "/default-avatar.png"} alt="" />
                </div>
                <div className="flex-1 space-y-1">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-gray-900 dark:text-white">{comment.user.name}</h3>
          ...
          <p className="text-center text-gray-500 dark:text-gray-400">{t("comments.noComments")}</p>
        )}
      </div>
    </div>
  )
