  ...
  const getTextAnalytics = (data) => {
    ...
    return {
      ...
      data: {
        ...
        averageLength: Math.round(validResponses.reduce((acc, text) => acc + text.length, 0) / validResponses.length),
      },
    }
  }
    ...
    switch (analytics.type) {
      ...
      case "pie":
        return (
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={analytics.data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(analytics.data).map(([key, value]) => (
              <div key={key} className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <div className="text-sm text-gray-500 dark:text-gray-400">{t(`templates.analytics.stats.${key}`)}</div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <div className="text-sm text-gray-500 dark:text-gray-400">{t("templates.analytics.totalResponses")}</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">{responses.length}</div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
              <div className="text-sm text-gray-500 dark:text-gray-400">{t("templates.analytics.completionRate")}</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
      ...
      {template.questions.map((question) => {
        ...
        return (
          <div key={question.id} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{question.title}</h3>
