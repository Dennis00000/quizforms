...
const SearchFilters = ({ currentTopic, currentTags, currentSort }) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{t("search.topic")}</h3>
      ...
      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{t("search.tags")}</h3>
        <TagInput value={currentTags || []} onChange={(tags) => updateFilter("tags", tags.join(","))} />
      </div>

      <div>
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{t("search.sortBy")}</h3>
        <select
          value={currentSort}
