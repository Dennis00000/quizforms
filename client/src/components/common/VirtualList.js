...
const VirtualList = ({
  ...
  if (!items.length) {
    return <div className={`flex items-center justify-center p-4 text-gray-500 ${className}`}>{emptyMessage}</div>
  }

  // Render only the visible items
  const visibleItems = items.slice(visibleRange.start, visibleRange.end + 1).map((item, index) => ({
    ...item,
  ...
  return (
    <div ref={containerRef} className={`overflow-auto relative ${className}`} style={{ height: "100%" }}>
      <div className="absolute w-full top-0 left-0" style={{ height: `${containerHeight}px` }}>
