...
const TemplateManagement = () => {
  ...
  const handleStatusChange = async (templateId, newStatus) => {
    try {
      ...
      // Update local state
      setTemplates(
        templates.map((template) => (template.id === templateId ? { ...template, status: newStatus } : template)),
      )
  ...
  return (
    <div>
      <div className="sm:flex sm:items-center sm:justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{t("admin.templates.title")}</h2>
      </div>
