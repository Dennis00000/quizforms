...
const UserManagement = () => {
  ...
  const handleSaveUser = async (e) => {
    ...
    try {
      if (selectedUser) {
        ...
        setUsers((prev) => prev.map((user) => (user.id === selectedUser.id ? { ...user, ...userForm } : user)))
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Edit User Modal */}
      <Modal isOpen={showUserModal} onClose={() => setShowUserModal(false)} title={t("admin.editUser")}>
        <form onSubmit={handleSaveUser} className="p-6">
          ...
          <div className="mb-4">
            ...
            />
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{t("admin.emailChangeNotice")}</p>
          </div>
          ...
          <div className="mb-4">
            ...
            <select id="role" name="role" value={userForm.role} onChange={handleUserFormChange} className="form-input">
              <option value="user">{t("admin.userRole")}</option>
              <option value="admin">{t("admin.adminRole")}</option>
            </select>
          </div>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} title={t("admin.deleteUser")}>
        <div className="p-6">
