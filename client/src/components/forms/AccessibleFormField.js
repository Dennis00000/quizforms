 ...
 */
const AccessibleFormField = ({ label, type = "text", error, required = false, helpText, children }) => {
  const id = useId()
  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between">
        <label htmlFor={id} className={`block text-sm font-medium ${error ? "text-red-700" : "text-gray-700"}`}>
          {label}
          {required && (
