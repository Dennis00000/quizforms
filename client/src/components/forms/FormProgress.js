 ...
 */
const FormProgress = ({ totalSteps, currentStep, stepLabels = [], onStepClick }) => {
  return (
              ...
              <div className="flex items-center">
                <button
                  ...
                  className={`relative flex h-8 w-8 items-center justify-center rounded-full ${
                    isCompleted ? "bg-indigo-600 hover:bg-indigo-800" : isActive ? "bg-indigo-600" : "bg-gray-300"
                  }`}
                  aria-label={`Step ${stepNumber}${stepLabels[index] ? `: ${stepLabels[index]}` : ""}`}
                ...
                {index !== totalSteps - 1 && (
                  <div
                    className={`absolute top-4 h-0.5 w-full max-w-20 ${isCompleted ? "bg-indigo-600" : "bg-gray-300"}`}
                    style={{ left: "2rem" }}
                  />
                )}
              </div>
