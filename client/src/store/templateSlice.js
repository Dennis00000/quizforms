...
export const fetchTemplates = createAsyncThunk(
  ...
  async (
    { page = 1, limit = 10, search = "", category = "", sortBy = "created_at", sortOrder = "desc" },
    { rejectWithValue },
  ) => {
    try {
      ...
      let query = supabase.from("templates").select("*, profiles(name)", { count: "exact" })
...
export const fetchTemplateById = createAsyncThunk("templates/fetchTemplateById", async (id, { rejectWithValue }) => {
  try {
    const { data, error } = await supabase.from("templates").select("*, profiles(name)").eq("id", id).single()
...
export const updateTemplate = createAsyncThunk(
  ...
  async ({ id, templateData }, { rejectWithValue }) => {
    try {
      const { data, error } = await supabase.from("templates").update(templateData).eq("id", id).select().single()
...
export const deleteTemplate = createAsyncThunk("templates/deleteTemplate", async (id, { rejectWithValue }) => {
  try {
...
export const { setCurrentTemplate, clearCurrentTemplate, setFilters, resetFilters, clearError } = templateSlice.actions
