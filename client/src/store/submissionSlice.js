...
export const fetchSubmissions = createAsyncThunk(
  ...
  async (
    { page = 1, limit = 10, search = "", templateId = "", status = "", sortBy = "created_at", sortOrder = "desc" },
    { rejectWithValue },
  ) => {
    try {
      ...
      let query = supabase.from("submissions").select("*, templates(title)", { count: "exact" })
...
export const fetchSubmissionById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const { data, error } = await supabase.from("submissions").select("*, templates(*)").eq("id", id).single()
...
export const updateSubmission = createAsyncThunk(
  ...
  async ({ id, submissionData }, { rejectWithValue }) => {
    try {
      const { data, error } = await supabase.from("submissions").update(submissionData).eq("id", id).select().single()
...
export const deleteSubmission = createAsyncThunk("submissions/deleteSubmission", async (id, { rejectWithValue }) => {
  try {
...
export const { setCurrentSubmission, clearCurrentSubmission, setFilters, resetFilters, clearError } =
