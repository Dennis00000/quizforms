  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      ...
      {loading ? (
        ...
        <div>
          <p className="text-gray-600 dark:text-gray-400 mb-4">{t("search.resultsCount", { count: results.length })}</p>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {results.map((template) => (
              <div key={template.id} className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">{template.title}</h2>
        ...
        <div className="text-center py-12">
          <p className="text-gray-600 dark:text-gray-400">{t("search.noResults", { query })}</p>
        </div>
      ) : (
        <div className="text-center py-12">
          <p className="text-gray-600 dark:text-gray-400">{t("search.enterQuery")}</p>
        </div>
      )}
    </div>
  )
