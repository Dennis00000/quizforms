...
class CacheManager {
    ...
    predictedRoutes.forEach((route) => {
      if (route.startsWith("/templates/")) {
        ...
        queryClient.prefetchQuery(["templates", templateId], () =>
          import("../services/api/templateApi").then((m) => m.templateApi.getTemplateWithQuestions(templateId)),
        )
      } else if (route === "/templates") {
        queryClient.prefetchQuery(["templates", "public"], () =>
          import("../services/api/templateApi").then((m) => m.templateApi.getPublicTemplates()),
        )
      }
    })
  }
  ...
  updateRelatedEntities(entityType, id, updatedData) {
    if (entityType === "templates") {
      ...
      this.updateEntityInCollection(["templates", "user"], (templates) =>
        templates.map((t) => (t.id === id ? { ...t, ...updatedData } : t)),
      )

      // Update template in public templates
      this.updateEntityInCollection(["templates", "public"], (templates) =>
        templates.map((t) => (t.id === id ? { ...t, ...updatedData } : t)),
      )
      ...
      if (submission) {
        ...
        this.updateEntityInCollection(["submissions", "template", templateId], (submissions) =>
          submissions.map((s) => (s.id === id ? { ...s, ...updatedData } : s)),
        )

        // Update in user submissions
        this.updateEntityInCollection(["submissions", "user", userId], (submissions) =>
          submissions.map((s) => (s.id === id ? { ...s, ...updatedData } : s)),
        )
      }
    }
