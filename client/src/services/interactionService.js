...
class InteractionService {
  ...
  async getTags() {
    const { data, error } = await supabase.from("tags").select("name, count").order("count", { ascending: false })
  ...
  async likeTemplate(templateId) {
    const { data, error } = await supabase.from("template_likes").insert([{ template_id: templateId }])
  ...
  async unlikeTemplate(templateId) {
    const { error } = await supabase.from("template_likes").delete().match({ template_id: templateId })

    if (error) throw error
  }

  async addComment(templateId, content) {
    const { data, error } = await supabase.from("comments").insert([{ template_id: templateId, content }])
