...
export function SubmissionProvider({ children }) {
  ...
  const fetchTemplateSubmissions = useCallback(
    async (templateId) => {
      if (!user) return
  ...
  const refreshTemplateSubmissions = useCallback(
    async (templateId) => {
      if (!user) return
  ...
  const deleteSubmission = useCallback(
    async (submissionId) => {
      ...
      try {
        ...
        const { error } = await supabase.from("submissions").delete().eq("id", submissionId).eq("user_id", user.id) // Ensure user can only delete their own
